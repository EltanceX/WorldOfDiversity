...
public class OriginalCommunityContentScreen : Screen
{
	...
	public OriginalCommunityContentScreen()
	{
		...
		{
			if (m_listPanel.SelectedItem != null && !(m_listPanel.SelectedItem is OriginalCommunityContentEntry))
			{
	...
	public override void Update()
	{
		...
		m_deleteButton.IsEnabled = UserManager.ActiveUser != null && communityContentEntry != null && m_filter as string == UserManager.ActiveUser.UniqueId;
		...
		if (m_populatingListCount > 0 && m_busyDialog.RootWidget != ScreensManager.RootWidget)
		{
	...
	public void DeleteEntry(OriginalCommunityContentEntry entry)
	{
		...
		DialogsManager.ShowDialog(null, new MessageDialog(LanguageControl.Get(fName1, "4"), LanguageControl.Get(fName1, "5"), LanguageControl.Yes, LanguageControl.No, delegate(MessageDialogButton button)
		{
			if (button == MessageDialogButton.Button1)
			{
				...
				{
					DialogsManager.HideDialog(busyDialog);
					DialogsManager.ShowDialog(null, new MessageDialog(LanguageControl.Get(fName1, "6"), LanguageControl.Get(fName1, "7"), LanguageControl.Ok, null, null));
				}, delegate(Exception error)
				{
					DialogsManager.HideDialog(busyDialog);
					DialogsManager.ShowDialog(null, new MessageDialog(LanguageControl.Error, error.Message, LanguageControl.Ok, null, null));
