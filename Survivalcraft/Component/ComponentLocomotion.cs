...
namespace Game
{
    public class ComponentLocomotion : Component, IUpdateable
    {
        ...
        public virtual void NormalMovement(float dt)
        {
            ...
            if (WalkSpeed > 0f && WalkOrder.HasValue)
            {
                ...
                else
                {
                    ...
                    if (m_componentPlayer != null && AllowShoesWarning && Time.PeriodicEvent(10.0, 0.0) && (m_shoesWarningTime == 0.0 || Time.FrameStartTime - m_shoesWarningTime > 300.0) && m_componentCreature.ComponentBody.StandingOnValue.HasValue && m_componentCreature.ComponentBody.ImmersionFactor < 0.1f)
                    {
                        ...
                        ClothingData clothingData = BlocksManager.Blocks[Terrain.ExtractContents(value2)].GetClothingData(value2);
                        if (clothingData != null)
                        {
                            flag = clothingData.MovementSpeedFactor > 1f;
                        }
            ...
            BasicHook.OnNormalMovementJump(this, ref velocity);
            if (JumpOrder > 0f && (m_componentCreature.ComponentBody.StandingOnValue.HasValue || m_componentCreature.ComponentBody.ImmersionFactor > 0.5f) && !m_componentCreature.ComponentBody.IsCrouching)
            {
                ...
                //velocity.Y = 4.5f;
                //ScreenLog.Info($"2. {velocity.Y}, {num6}");
                m_jumping = true;
