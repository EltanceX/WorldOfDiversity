...
namespace Game
{
    public class ComponentPickableGathererPlayer : ComponentPickableGatherer, IUpdateable
    {
        ...
        {
            if(m_componentHealth.Health <= 0) return false;
            if (pickable.FlyToPosition.HasValue) return false;
            if (pickable.ToRemove) return false;
            ...
            if (pickable.Value == m_experienceBlockIndex) return true;
        ...
        public override void GatherPickable(Pickable pickable)
        {
            ...
            if (!pickable.ToRemove)
            {
                ...
                if (pickable.Count == 0)
                {
                    ...
                    m_subsystemAudio.PlaySound("Audio/PickableCollected", 0.7f, -0.4f, Position, 2f, autoDelay: false);
                }
            }
        }
        ...
        {
            if (m_inventory == null) m_inventory = m_componentPlayer.ComponentMiner.Inventory;
            base.Update(dt);
        }
