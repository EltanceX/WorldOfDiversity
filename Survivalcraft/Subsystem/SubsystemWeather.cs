...
namespace Game
{
	public class SubsystemWeather : Subsystem, IDrawable, IUpdateable
	{
		...
		public virtual float PrecipitationIntensity
		{
			get;
			set;
		}
		...
		m_subsystemBlocksScanner.ScanningChunkCompleted += delegate(TerrainChunk chunk)
		{
			...
			{
				FreezeThawAndDepositSnow(chunk, 0.66f, 0.66f, forceDepositSnow: false);
			}
		};
		...
		public virtual void UpdateLightning(float dt)
		{
			...
			for (int i = 0; i < allocatedChunks.Length; i++)
			{
				...
				if (vector.HasValue)
				{
					SubsystemSky.MakeLightningStrike(vector.Value, manual: false);
					break;
				}
			}
		}
		...
		public virtual void FreezeThawAndDepositSnow(TerrainChunk chunk, float freezeProbability, float thawProbability, bool forceDepositSnow)
		{
			...
			for (int j = 0; j < m_shuffledOrder.Length; j++)
			{
				...
				if (precipitationShaftInfo.Type == PrecipitationType.Snow)
				{
					...
					if (num4 == 18 && SubsystemTerrain.TerrainContentsGenerator.CalculateOceanShoreDistance(num5, num7) > -35f)
					{
				...
				else
				{
					...
					for (; num6 > 0; num3--, num6--, cellValueFast = chunk.GetCellValueFast(num, num3, num2), num4 = Terrain.ExtractContents(cellValueFast))
					{
						switch (num4)
						{
							case 61:
								SubsystemTerrain.DestroyCell(0, num5, num6, num7, 0, noDrop: true, noParticleSystem: true);
								continue;
							case 62:
								SubsystemTerrain.DestroyCell(0, num5, num6, num7, 0, noDrop: false, noParticleSystem: true);
								continue;
						}
						break;
					}
