				...
				for (int j = y2; j <= y2; j++)
				{
					...
					for (int k = 0; k < 12; k++)
					{
						...
						for (int l = 0; l < num10; l++)
						{
							...
							int value1 = terrain.GetCellValueFast(num11, num9, num12);
							if (BlocksManager.Blocks[Terrain.ExtractContents(value1)].IsCollidable_(value1))
							{
								flag = false;
								break;
							}
							int value2 = terrain.GetCellValueFast(num11, num9 - 1, num12);
							if (BlocksManager.Blocks[Terrain.ExtractContents(value2)].IsCollidable_(value2))
							{
						...
						if (treeType.HasValue)
						{
							...
							for (int m = 0; m < num10; m++)
							{
								...
								if (m > num10 / 2)
								{
									...
									if (random.Bool(0.5f) && !BlocksManager.Blocks[Terrain.ExtractContents(value3)].IsCollidable_(value3))
									{
									...
									if (random.Bool(0.5f) && !BlocksManager.Blocks[Terrain.ExtractContents(value5)].IsCollidable_(value5))
									{
										terrain.SetCellValueFast(num13 - point2.X, num9, num14 - point2.Z, treeLeavesValue);
									}
									...
									if (random.Bool(0.5f) && !BlocksManager.Blocks[Terrain.ExtractContents(value7)].IsCollidable_(value7))
									{
					...
					for (int n = 0; n < 32; n++)
					{
						...
						num19++;
						int value8 = terrain.GetCellValueFast(num17 + 1, num19, num18);
						int value9 = terrain.GetCellValueFast(num17 - 1, num19, num18);
						int value10 = terrain.GetCellValueFast(num17, num19, num18 + 1);
						int value11 = terrain.GetCellValueFast(num17, num19, num18 - 1);

						if (!BlocksManager.Blocks[Terrain.ExtractContents(value8)].IsCollidable_(value8) && !BlocksManager.Blocks[Terrain.ExtractContents(value9)].IsCollidable_(value9) && !BlocksManager.Blocks[Terrain.ExtractContents(value10)].IsCollidable_(value10) && !BlocksManager.Blocks[Terrain.ExtractContents(value11)].IsCollidable_(value11))
						{
