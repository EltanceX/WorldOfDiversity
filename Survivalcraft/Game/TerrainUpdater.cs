...
namespace Game
{
    ...
    {
        public void UpdateFlareLightSource(TerrainChunk chunk)
        {
            var subsystemProjectiles = m_subsystemTerrain.Project.FindSubsystem<SubsystemProjectiles>();
            if (subsystemProjectiles == null) return;
#if GDEBUG
            ScreenLog.Info($"Update Flare LightSource: {chunk.Coords.X}, {chunk.Coords.Y} [Flares: {subsystemProjectiles.Projectiles.Count}]");
#endif
            foreach (var projectile in subsystemProjectiles.Projectiles)
            {
                if (projectile is not FlareProjectile) continue;
                FlareProjectile flareProjectile = (FlareProjectile)projectile;
                //if (flareProjectile.LightChunk == null || flareProjectile.LightChunk != chunk) return;\
                if (flareProjectile.ToRemove || flareProjectile.Trajectory.Count == 0) continue;
                var newpos = flareProjectile.Trajectory.Last();
                if (newpos == null || newpos.Length < 3) continue;
                this.m_lightSources.Add(new TerrainUpdater.LightSource
                {
                    X = newpos[0],
                    Y = Math.Clamp(newpos[1], 0, 256),
                    Z = newpos[2],
                    Light = 14
                });
            }
        ...
        {
#if GDEBUG
            ScreenLog.Info($"UpdateChunkSingleStep: {chunk.Coords.X}, {chunk.Coords.Y} [{chunk.State}, {chunk.ThreadState}]");
#endif
                ...
                case TerrainChunkState.InvalidPropagatedLight:
                    {
                        ...
                        //EntryHook To be added
                        UpdateFlareLightSource(chunk);
        ...
        {
#if GDEBUG
            ScreenLog.Info($"PropagateLightSources: {m_lightSources.Count}");
#endif
