...
public class TerrainContentsGenerator24 : ITerrainContentsGenerator
{
    ...
    public TerrainContentsGenerator24(SubsystemTerrain subsystemTerrain)
    {
        ...
        ChunkGenerationStep4.Add(new ChunkGenerationStep(1600, GenerateRuins));
        ModsManager.HookAction("TerrainContentsGenerator24Initialize", loader =>
        {
            loader.TerrainContentsGenerator24Initialize(this, subsystemTerrain);
            return false;
        });
    ...
    public virtual void GenerateRuins(TerrainChunk chunk)
    {
        RuinsGenerator.OnChunkGenerate(chunk,this);
    }
    ...
    public virtual float ScoreSpawnPosition(int x, int z)
    {
        ...
        float x2 = CalculateMountainRangeFactor(x, z);
        float num3 = CalculateHeight(x, z);
        float x3 = CalculateHeight(x - 8, z - 8);
        float x4 = CalculateHeight(x - 8, z + 8);
        float x5 = CalculateHeight(x + 8, z - 8);
        float x6 = CalculateHeight(x + 8, z + 8);
        float num4 = MathUtils.Min(num3, MathUtils.Max(x3, x4, x5, x6));
        float x7 = MathUtils.Max(num3, MathUtils.Max(x3, x4, x5, x6)) - num4;
        float num5 = 0f;
        int num6 = 0;
        for (int i = -4; i <= 4; i += 2)
        {
            ...
            for (int j = -1; j <= 1; j++)
            {
                ...
                if (random.Bool(0.02f + 0.01f * num4))
                {
                    ...
                    for (int num15 = 0; num15 < num14; num15++)
                    {
                        ...
                        int num16 = 8 + (int)MathF.Round(vector.X);
                        int num17 = 0;
                        int num18 = 8 + (int)MathF.Round(vector.Y);
                        m_waterPocketBrushes[random.Int(0, m_waterPocketBrushes.Count - 1)].PaintFast(chunk, num11 + num16, num12 + num17, num13 + num18);
                    }
                }
                if (random.Bool(0.06f + 0.05f * num4))
                {
                    ...
                    for (int num23 = 0; num23 < num22; num23++)
                    {
                        ...
                        int num24 = 8 + (int)MathF.Round(vector2.X);
                        int num25 = random.Int(0, 1);
                        int num26 = 8 + (int)MathF.Round(vector2.Y);
                        m_magmaPocketBrushes[random.Int(0, m_magmaPocketBrushes.Count - 1)].PaintFast(chunk, num19 + num24, num20 + num25, num21 + num26);
                    }
                }
            }
        }
    ...
    public virtual void GenerateTrees(TerrainChunk chunk)
    {
        ...
        for (int i = x; i <= x; i++)
        {
            for (int j = y; j <= y; j++)
            {
                ...
                float num2 = CalculateForestDensity(i * 16, j * 16);
                int num3 = (int)(6f * num2);
                int num4 = 0;
                for (int k = 0; k < 36; k++)
                {
                    if (num4 >= num3)
                    ...
                    {
                        TreeType? treeType = PlantsManager.GenerateRandomTreeType(random, num + SubsystemWeather.GetTemperatureAdjustmentAtHeight(num7), humidity, num7);
                        if (treeType.HasValue)
                        {
                            ReadOnlyList<TerrainBrush> treeBrushes = PlantsManager.GetTreeBrushes(treeType.Value);
